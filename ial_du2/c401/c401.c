...
**/
	*RootPtr = NULL; // set tree's root value to NULL
}
...
**/
	if ((*RootPtr) == NULL) { // if tree doesn't exists
...
**/
	...
	int Content;
	if (BSTSearch((*RootPtr), K, &Content) == FALSE) {
		return;
	}
	...
	if (K == (*RootPtr)->Key) {
		if ((*RootPtr) -> LPtr != NULL && (*RootPtr)->RPtr != NULL) {
			...
			*RootPtr = NULL;
			...
			tBSTNodePtr to_delete = (*RootPtr) -> LPtr;
			free(*RootPtr);
			*RootPtr = to_delete;
		}else{
			tBSTNodePtr to_delete = (*RootPtr) -> RPtr;
			free(*RootPtr);
			*RootPtr = to_delete;
		}
	}else if (K > (*RootPtr)->Key) {
