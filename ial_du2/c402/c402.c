...
**/
	tBTNodePtr pred_ptr = NULL, next_ptr = *RootPtr;

	while (next_ptr != NULL) {
		pred_ptr = next_ptr;

		if (Content == next_ptr->Cont) {
			return;
		}else if (Content > next_ptr->Cont) {
			next_ptr = next_ptr->RPtr;
		}else{
			next_ptr = next_ptr->LPtr;
		}
	}
	...
	new_node->RPtr = new_node->LPtr = NULL;
	...
	if (Content > pred_ptr->Cont) {
		pred_ptr->RPtr = new_node;
	}else{
		pred_ptr->LPtr = new_node;
	}
}
/*                                  PREORDER                                  */
